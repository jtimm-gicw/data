...
const sortByChildren = (charArray) => {
  return charArray.sort((a, b) => { // sort through the charArray using a and b as a parameters because it will compare 2 objects in the array at a time
    const childCountA = a.children.length; //  This counts how many children character a has.
    const childCountB = b.children.length; //  This counts how many children character b has.

    if (childCountA !== childCountB) {
      return childCountA - childCountB; // sort by fewest children
    } else {
      return a.house.localeCompare(b.house); // tie-breaker: sort alphabetically by house
    }
  });
};
...
const containsW = (str) => {
  let regex = /w/; // does it contain a W
...
const isCapitalized = (str) => {
  let thatRegex= /[A-Z][A-Za-z]*/g;
  let strRegex = str.match(thatRegex);
    return strRegex || [];// will return a str or empty so that it does not return error
};
...
const citiesAtoJ = (arr) => {
  return arr.filter(city => /^[A-J]/i.test(city)); // .filter() — to create a new array based on a condition.
};
// .test() is a method that checks if the string (in this case, city) matches the pattern.  so this is testing if the city starts w/ A-J
// It returns true if it matches.
// It returns false if it doesn't.

// ^ means “start of the string.”

// [A-J] means any letter from A to J.

// /i makes it case-insensitive (so it works with "a" and "A").
