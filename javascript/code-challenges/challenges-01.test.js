...
const addOne = (arr) => {
  let newArr = []; // initialize an empty array newArr to store the incremented values.
  arr.forEach(num => { //use forEach to loop over 'each' number in the input array. 
  // forEach is used in this function to iterate over each element in the input array and apply an operation 
    newArr.push(num + 1); // Inside the loop, we add 1 to each number and push the result into newArr.
  });

  return newArr; // eturn the new array containing the incremented values.
};
...
const addExclamation = (arr) => {
  let newArr = [];
  arr.forEach(str => {
    newArr.push(str + "!"); // Add "!" to each string and push to newArr
  });
...
const allUpperCase = (arr) => {
  let newArr = []; 
  arr.forEach(str => {
    newArr.push(str.toUpperCase()); // Convert string to uppercase and push to newArr
  });
...
------------------------------------------------------------------------------------------------ */
  ...
  const speaker = (words, callback) => {
    ...
    words.forEach(word => {
      newArray.push(callback(word)); // Apply callback to each word and store result
    });
...
const addNumbers = (num, arr, times, callback) => {
  for (let i = 0; i < times; i++) { // Loop 'times' times
    callback(arr, num); // Call the callback function to add 'num' to 'arr'
  }
  ...
  const createList = (availableItems) => {
    ...
    availableItems.forEach(item => {
      if (item.available) { // Check if the item is available
        groceryList.push(item.name); // Add the item name to the grocery list
      }
    });
  
    return groceryList; // Return the final list of available items
  };
