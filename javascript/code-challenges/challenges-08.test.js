...
const sortStarWarsCharacters = (starWarsArr) => {
  return starWarsArr.sort((a,b) => Number(b.height)-Number(a.height)); // b is the largest number
};
...
const howMuchPencil = (str) => {
  ...
  // Loop from 0 up to the length of the string
  for (let i = 0; i <= str.length; i++) {
    // At each step, slice the string from index i and push it to the result
    result.push(str.slice(i));
  }

  return result;
};
// Output: ['Welcome', 'elcome', 'lcome', 'come', 'ome', 'me', 'e', '']
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ...
    // Find the second space (after the unit)
    let secondSpace = item.indexOf(' ', firstSpace + 1);

    // Slice from after the second space to the end (the food name)
    let foodName = item.slice(secondSpace + 1);
