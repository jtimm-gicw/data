...
const raisedToTheThird = (arr) => {
  let newArray =[];
  arr.forEach(num => {
    newArray.push(Math.pow(num, 3)); // Raise to 3rd power and push to newArray
  });
  return newArray;
};
...
const forLoopTwoToThe = (arr) => {
  let loopArr = []; // Step 1: Create an empty array
  
  for (let i = 0; i < arr.length; i++) { // Step 2: Loop through each element
    loopArr.push(Math.pow(2, arr[i])); // Step 3 & 4: Raise 2 to the power of arr[i] and push to loopArr
  }

  return loopArr; // Step 5: Return the final array
};

// HOW IT WORKS:
// for (let i = 0; i < arr.length; i++) → Loops through the array.
// arr[i] → Accesses each number in the array.
// Math.pow(2, arr[i]) → Calculates 2 raised to that number.
// loopArr.push(...) → Stores the result in loopArr.
// return loopArr; → Returns the new array after looping.
...
// Store the result in loopArr.
// Return the final array with all calculated values.

// EXPLAINATION:
// Math.pow(2, num) → This means 2 raised to the power of num.
// .push() → Adds the new value to the array.
// return loopArr; → This gives back the final array.
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const mapTwoToThe = (arr) => {
  return arr.map(num => Math.pow(2, num)); // No need for push, map handles it
};
// NOTE:
// (num, 2) - this order means "num squared"
// (2, num) - this order means "num to the power of..."
