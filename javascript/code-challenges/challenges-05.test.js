...
const maxInArray = (arr) => {
  Math.max(...arr); // ...arr changes the array to having an OUTPUT: 9
};
...
const getCourseKeys = (obj) => {
  return Object.keys(obj); // extracts an array of the object's top-level property names (keys).
};
...
const checkValues = (obj, value) => {
  let found = false;
  Object.values(obj).forEach(val => val === value ? found = true : '');// Loops through each value in that array. If the current value matches value, it sets found to true.
  return found;
};
...
const updateNumbers = (obj) => {
  ...
  Object.keys(obj).forEach(key => newArr.push(`${key}
    : ${obj[key]}`));// This gets an array of all the keys in the object.
    // Loops through each key.
    // Pushes this string into the newArr array.
...
// .map() goes through each object in the arr (which is the characters array)
// It extracts the house property from each character
// It creates a new array containing all the house names
...
const hasChildrenValues = (arr, character) => {
  ...
  const person = arr.find(char => char.name === character);

  // If character not found, return false
  if (!person) return false;

  // Check if 'children' exists in the object's values
  return Object.values(person).includes(person.children); // Object.values(obj) returns an array of all values in an object.
  // If the character has a children property, it will appear in Object.values(obj).
}; 
